...
- **Town 模式** — 3D 低多边形小镇，实时观看 NPC 生活、社交、做手艺（首次点击才加载，节省资源）
- **Chat 模式** — IM 风格聊天界面，左侧 Agent 列表（管家 + 居民，在线状态），右侧消息流，支持文本/图片多模态、聊天历史（游标分页）、完整 OpenClaw 斜杠命令自动补全（40+ 命令：会话管理、模型控制、发现、技能、子代理、管理、语音）、群聊 @提及、页眉内联思考级别 / 推理可见性下拉框。应用默认路由。
- **Claw 设置** — 应用内面板，管理 OpenClaw 运行时配置，左侧导航 11 个分组：通用设置（Gateway 模式、子 Agent 超时、默认模型）、系统（日志、更新、诊断、审计）、消息（会话、消息、命令、定时任务）、工具（浏览器、工具、网页、媒体、MCP）、AI（语音、转录、承诺、广播、ACP）、网络（记忆、代理、环境变量、钩子、界面），以及代理管理（每居民模型代理）、模型管理（LLM 提供商/模型增删改查）、插件配置、会话列表（实时 token 用量）、关于——所有默认值与 `openclaw.json` 保持一致
...
- **自由生活模拟** — 居民自主生活：社交、做手艺、逛小镇——没有中央调度，就是邻居过日子
- **实时对话气泡** — AI 的回复以打字机效果显示在 NPC 头顶，支持流式更新和分页
- **独立居民** — 每个居民都是独立 Agent，有自己的手艺和日常节奏；管家是小镇向导，不是任务调度器
...
- **4 轨 BGM** — day / dusk / night / life 四首背景音乐，按天气、时段、场景自动切换，3.5 秒交叉淡化
- **NPC 日常行为（双模式）** — 默认算法驱动：DailyBehavior 状态机 + 5 种行为模板（按专业匹配） + 400+ 条预设台词社交，零 LLM 开销；开启灵魂模式后切换为 AI 驱动：AgentBrain 三层决策（L1 日计划 / L2 战术 / L3 对话）+ LLM 深度多轮对话 + 关系图谱 + 每日叙事摘要。可通过「居民自动行走」开关暂停/恢复定时行走，不移除任何居民。
- **居民独立聊天** — 点击居民 NPC 直接发起对话，消息路由到居民自己的独立 Agent 会话
- **群聊** — 多居民群聊对话，支持 @提及选择器、居民职位显示、JSONL 历史持久化、上下文 token 预算跟踪
- **话题讨论** — 发起多居民参与的群组话题讨论，支持结构化轮流发言和 AI 主持。话题状态刷新页面后自动恢复（召集的居民和控制按钮），讨论期间居民需求与经济数值冻结
- **小镇烦恼事件** — 居民偶尔会遇到小烦恼（漏雨的屋顶、走失的猫、邻里口角），头顶冒出烦恼气泡——点击安抚，看心情变好。含 combo 连击、NPC 心情系统，可扩展更多玩法
...
- **居民空间感知与移动** — 居民可感知 3D 世界并移动其中，3 个 AI 工具：获取自身状态/位置、查询半径范围内居民、行走至坐标——由 plugin↔frontend NPC 查询通道驱动
- **编辑器预览** — 一键打开游戏级预览窗口（WASD 控制 + 完整昼夜天气 + 车辆动画 + 音频）
- **LLM 模型管理** — 直接在 Claw 设置面板内管理 `openclaw.json` 提供商/模型：增删改查、导入导出、撤销重做——无需独立页面
- **居民模型代理管理** — 每居民（Agent）LLM 模型代理管理：主模型 + 备用模型、身份（名称、Emoji）、思考/推理默认值、上下文 Token、子 Agent 超时、群聊历史条数——全部映射到 `openclaw.json` `agents.list[]`
- **灵魂系统** — 每个 NPC 有一份 Markdown 人设文件，定义性格、说话风格、手艺和日常生活

### 交互与视觉

- **交互式 3D** — 点击 NPC 查看状态卡片（头像/人格/活动日志/thinking 流/TODO），拖拽视角，滚轮缩放
- **三场景切换** — 小镇（日常生活）/ 工坊（手艺工坊）/ 展示厅（开发中），带淡入淡出过渡和 NPC 迁移
- **丰富的 VFX** — 人格变换魔法阵、思考光环、搜索雷达、部署彩纸、心情变好闪光…
- **10 工作台工坊** — 每个工作台有独立显示器（实时手艺动画），NPC 进场/做手艺/离场全流程编排
- **手艺成果预览** — 手艺完成后弹出成果卡片，支持图片（lightbox 放大）、视频、音频预览和下载，游戏/网站类直接 iframe 启动
- **AI 工具控制** — Agent 可以通过工具主动控制小镇（广播消息、生成 NPC、触发特效、设置时间/天气，编辑地图：放置/移动/变换/删除物件、设置地形、扩展地图，现已支持感知/移动 3D 空间：获取状态、查询附近居民、行走至坐标）
...
每个 NPC 都有独立的**灵魂文件**（Markdown），定义了角色的性格、说话风格、手艺和日常生活。

灵魂文件格式：以 `# 角色名` 开头的 Markdown 文件，可包含元数据（模型 id、性别、手艺）、人设核心、性格详细设定、对话示例等。内容会作为 system prompt 注入 Agent。
...
基础功能**不需要额外下载**——小镇、工坊、NPC 使用的内置模型（KayKit + Kenney）已包含在仓库中。
...
**现象**：管家 Agent 无法找到 `town_announce`、`town_spawn_npc` 等插件工具。
...
| 阶段 | 方向 | 状态 |
|------|------|------|
| **一座小镇** | 3D 可视化、灵魂系统、居民工坊、双模式界面 | ✅ 已实现 |
